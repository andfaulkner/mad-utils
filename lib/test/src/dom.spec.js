...
console.log("dom.spec.ts :: userAgentParsed:", browser_1.userAgentParsed);
...
describe("dom sub-module", function () {
    ...
    test_1.expectNonEmptyObjectExists(browser_1.userAgentParsed, "dom->userAgentParsed");
    describe("userAgentParsed", function () {
        it("should contain keys ua, browser, engine, os, device, cpu, raw, ua", function () {
            chai_1.expect(browser_1.userAgentParsed).to.contain.keys([
                "ua",
                "browser",
                "engine",
                "os",
        ...
        it("should have objects at all keys except raw & ua (which should be strings)", function () {
            Object.keys(browser_1.userAgentParsed).forEach(function (key) {
                if (key === "raw" || key === "ua") {
                    chai_1.expect(browser_1.userAgentParsed[key]).to.be.a("string");
                }
                else {
                    chai_1.expect(browser_1.userAgentParsed[key]).to.be.a("object");
                    chai_1.expect(browser_1.userAgentParsed[key]).to.not.be.a("string");
                    chai_1.expect(Object.keys(browser_1.userAgentParsed[key]).length).to.be.greaterThan(0);
                }
            });
        });
    });
