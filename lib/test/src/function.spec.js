...
describe("function sub-module", function () {
    ...
    describe("throttle", function () {
        ...
        it("runs function again if request to run occurs after [wait]ms elapses", function () {
            var val = 0;
            var throttledFn = shared_1.throttle(function () { return val++; }, 50, true);
            throttledFn();
            setTimeout(function () { return throttledFn(); }, 100);
            throttledFn();
            throttledFn();
            setTimeout(function () { return chai_1.expect(val).to.eql(2); }, 150);
        });
        it("requires the correct arguments for the created 'throttled' function", function () {
            var arr = [];
            var throttledFn = shared_1.throttle(function (str) { return arr.push(str); }, 50, true);
            throttledFn('First');
            setTimeout(function () { return throttledFn('Second'); }, 100);
            throttledFn("WON'T BE INCLUDED");
            throttledFn("ALSO WON'T BE INCLUDED");
            setTimeout(function () { return chai_1.expect(arr).to.eql(['First', 'Second']); }, 150);
        });
        it("transfers data keys from given function to throttled function", function () {
            var arr = [];
            var fn = Object.assign(function () { return arr.push(fn.dataVal); }, { dataVal: 'VALUE' });
            var throttledFn = shared_1.throttle(fn, 50, true);
            chai_1.expect(throttledFn.dataVal).to.eql('VALUE');
            throttledFn();
            throttledFn();
            setTimeout(function () { return throttledFn(); }, 100);
            setTimeout(function () { return chai_1.expect(arr).to.eql(['VALUE', 'VALUE']); }, 150);
        });
    });
    describe('getArgNames', function () {
        it("returns empty list given arrow function with no params", function () {
            var fn = function () { return console.log('output'); };
            chai_1.expect(shared_1.getArgNames(fn)).to.eql([]);
        });
        it("gets list of arguments from given arrow function with params", function () {
            var fn = function (arg1, arg2) { return console.log('output'); };
            chai_1.expect(shared_1.getArgNames(fn)).to.eql(['arg1', 'arg2']);
        });
    ...
    describe("runAll", function () {
        it("returns empty array given an empty array", function () {
            chai_1.expect(shared_1.runAll([])).to.eql([]);
        });
        it("returns return val of function in an array, if given array w/ 1 function", function () {
            chai_1.expect(shared_1.runAll([function () { return 'result'; }])).to.eql(['result']);
        });
        it("returns array of return vals from all functions in an array", function () {
            chai_1.expect(shared_1.runAll([function () { return 'result 1'; }, function () { return 'result 2'; }, function () { return 'result 3'; }])).to.eql([
                'result 1',
                'result 2',
                'result 3',
            ]);
        ...
        it("Passes args past 1st arg to all functions in array (as arguments)", function () {
            // prettier-ignore
            chai_1.expect(shared_1.runAll([
                function (name, age) { return "Hello " + age + "-year-old " + name + "!"; },
                function (name, age) { return "How are you doing, " + age + "-year-old " + name + "?"; },
            ], 'Meeka', 7)).to.eql([
