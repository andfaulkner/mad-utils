...
describe("url sub-module", function () {
    ...
    describe("swapMatchingURLPaths", function () {
        ...
        it("swaps new path val into URL paths matching entire given string. Ignores query string", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths('first', 'ONE', testUrl))
                .to.eql(testHost + "/ONE/second/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths('second', 'TWO', testUrl))
                .to.eql(testHost + "/first/TWO/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths('third', 'THREE', testUrl))
        ...
        it("swaps new path val into URL paths fully matching given RegExp. Ignores query string", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths(/[a-z]irs[a-z]/, 'ONE', testUrl))
                .to.eql(testHost + "/ONE/second/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths(/[a-z]eco[a-z]{2}/, 'TWO', testUrl))
                .to.eql(testHost + "/first/TWO/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths(/.+rd/, 'THREE', testUrl))
                .to.eql(testHost + "/first/second/THREE?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths(/fir[a-z]+$/, 'FIRST', testUrl))
        ...
        it("swaps all when there are multiple matching paths (ignoring query string)", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths(/.+/, 'MATCH', testUrl))
