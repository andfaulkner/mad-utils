...
describe("url sub-module", function () {
    ...
    describe(".swapLastURLPath", function () {
        it("swaps the last URL path and leaves the query params intact", function () {
            ...
            chai_1.expect(shared_1.swapLastURLPath('FINAL', testURL)).to.eql(baseTestURL + "/1/2/FINAL?" + testQueryStr);
        });
    });
    ...
    describe("swapMatchingURLPaths", function () {
        ...
        it("swaps new path val into URL paths matching entire given string. Ignores query string", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths('first', 'ONE', testUrl)).to.eql(testHost + "/ONE/second/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths('second', 'TWO', testUrl)).to.eql(testHost + "/first/TWO/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths('third', 'THREE', testUrl)).to.eql(testHost + "/first/second/THREE?" + testQuery);
        });
        it("swaps new path val into URL paths fully matching given RegExp. Ignores query string", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths(/[a-z]irs[a-z]/, 'ONE', testUrl)).to.eql(testHost + "/ONE/second/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths(/[a-z]eco[a-z]{2}/, 'TWO', testUrl)).to.eql(testHost + "/first/TWO/third?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths(/.+rd/, 'THREE', testUrl)).to.eql(testHost + "/first/second/THREE?" + testQuery);
            chai_1.expect(shared_1.swapMatchingURLPaths(/fir[a-z]+$/, 'FIRST', testUrl)).to.eql(testHost + "/FIRST/second/third?" + testQuery);
        });
        it("does not swap new path val into partially matching URL paths.", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths('firs', 'ONE', testUrl)).to.eql(testUrl);
            chai_1.expect(shared_1.swapMatchingURLPaths('secon', 'TWO', testUrl)).to.eql(testUrl);
            chai_1.expect(shared_1.swapMatchingURLPaths(/fir[a-z]/, 'ONE', testUrl)).to.eql(testUrl);
            chai_1.expect(shared_1.swapMatchingURLPaths(/[a-z]econ/, 'TWO', testUrl)).to.eql(testUrl);
        });
        it("swaps all when there are multiple matching paths (ignoring query string)", function () {
            chai_1.expect(shared_1.swapMatchingURLPaths(/.+/, 'MATCH', testUrl)).to.eql(testHost + "/MATCH/MATCH/MATCH?" + testQuery);
        });
    });
    describe("normalizeURLPathname", function () {
        it("returns empty string if given empty string", function () {
            chai_1.expect(shared_1.normalizeURLPathname("")).to.eql("");
        });
        it("add / before paths", function () {
            chai_1.expect(shared_1.normalizeURLPathname("asdf")).to.eql("/asdf");
            chai_1.expect(shared_1.normalizeURLPathname("asdf/123")).to.eql("/asdf/123");
        });
        it("replace // with /", function () {
            chai_1.expect(shared_1.normalizeURLPathname("//")).to.eql("/");
            chai_1.expect(shared_1.normalizeURLPathname("//asdf")).to.eql("/asdf");
            chai_1.expect(shared_1.normalizeURLPathname("//asdf//123/qwerty")).to.eql("/asdf/123/qwerty");
            chai_1.expect(shared_1.normalizeURLPathname("//asdf/123//qwerty")).to.eql("/asdf/123/qwerty");
        });
        it("remove trailing /", function () {
            chai_1.expect(shared_1.normalizeURLPathname("/a/")).to.eql("/a");
            chai_1.expect(shared_1.normalizeURLPathname("/asdf/qwerty/")).to.eql("/asdf/qwerty");
        });
        it("remove / before query params", function () {
            chai_1.expect(shared_1.normalizeURLPathname("/a/?dog=meeka")).to.eql("/a?dog=meeka");
            chai_1.expect(shared_1.normalizeURLPathname("/asdf/qwerty/?dog=meeka")).to.eql("/asdf/qwerty?dog=meeka");
            chai_1.expect(shared_1.normalizeURLPathname("/asdf/qwerty/123/?dog=meeka")).to.eql("/asdf/qwerty/123?dog=meeka");
            chai_1.expect(shared_1.normalizeURLPathname("/asdf/qwerty/123/okok/?dog=meeka")).to.eql("/asdf/qwerty/123/okok?dog=meeka");
        });
    ...
    describe("urlPathnameWithQuery", function () {
        it("should return pathname from given URL with protocol and no query", function () {
            chai_1.expect(shared_1.urlPathnameWithQuery("https://www.example.com/a/b/c")).to.eql("/a/b/c");
            chai_1.expect(shared_1.urlPathnameWithQuery("https://example.com/asdf")).to.eql("/asdf");
        });
        it("should return pathname from given URL with no protocol and no query", function () {
            chai_1.expect(shared_1.urlPathnameWithQuery("www.example.com/a/b/c")).to.eql("/a/b/c");
            chai_1.expect(shared_1.urlPathnameWithQuery("example.com/asdf")).to.eql("/asdf");
        });
        it("should return pathname & query from given URL with no protocol", function () {
            chai_1.expect(shared_1.urlPathnameWithQuery("www.example.com/a/b/c?k=v")).to.eql("/a/b/c?k=v");
            chai_1.expect(shared_1.urlPathnameWithQuery("example.com/asdf?key=val&key2=val2")).to.eql("/asdf?key=val&key2=val2");
        });
        it("should return pathname & query from given URL with protocol", function () {
            chai_1.expect(shared_1.urlPathnameWithQuery("https://www.example.com/a/b/c?k=v")).to.eql("/a/b/c?k=v");
            chai_1.expect(shared_1.urlPathnameWithQuery("https://example.com/asdf?key=val&key2=val2")).to.eql("/asdf?key=val&key2=val2");
        });
    });
    ...
    describe("extractFromUrl", function () {
        it("returns query string at end if 'query' given as urlParts arg", function () {
            chai_1.expect(shared_1.extractFromUrl("query", "http://www.exmpl.ca:8080/a/b/c?key=val&b=2")).to.eql("?key=val&b=2");
            chai_1.expect(shared_1.extractFromUrl(["query"], "http://www.exmpl.ca:8080/a/b/c?key=val&b=2")).to.eql("?key=val&b=2");
        });
        it("returns protocol (including ://) if 'protocol' given as urlParts arg", function () {
            chai_1.expect(shared_1.extractFromUrl("protocol", "http://www.exmpl.ca:8080/a/b/c?key=val&b=2")).to.eql("http://");
            chai_1.expect(shared_1.extractFromUrl(["protocol"], "https://www.exmpl.ca:8080/a/b/c?key=val&b=2")).to.eql("https://");
        });
        it("returns hostname if 'hostname' given as urlParts arg", function () {
            chai_1.expect(shared_1.extractFromUrl("hostname", "http://www.exmpl.ca:8080/a/b/c?key=val&b=2")).to.eql("www.exmpl.ca");
            chai_1.expect(shared_1.extractFromUrl(["hostname"], "https://www.exmpl2.ca:8080/a/b/c?key=val&b=2")).to.eql("www.exmpl2.ca");
        });
        it("returns port (including ':') if 'port' given as urlParts arg", function () {
            chai_1.expect(shared_1.extractFromUrl("port", "http://www.exmpl.ca:8080/a/b/c?key=val&b=2")).to.eql(":8080");
            chai_1.expect(shared_1.extractFromUrl(["port"], "https://www.exmpl2.ca:9000/a/b/c?key=val&b=2")).to.eql(":9000");
        });
        it("returns empty string if 'port' given as urlParts arg but URL has no port", function () {
            chai_1.expect(shared_1.extractFromUrl("port", "https://www.secondexample.com/a/b/c?key=val&b=2")).to.eql("");
            chai_1.expect(shared_1.extractFromUrl(["port"], "https://www.exmpl2.ca/a/b/c?key=val&b=2")).to.eql("");
        });
        it("returns pathname (e.g. /a/b/c) if urlParts arg is 'pathname' or 'path'", function () {
            chai_1.expect(shared_1.extractFromUrl("path", "https://www.example.com/asdf/qwer/last?key=val&b=2")).to.eql("/asdf/qwer/last");
            chai_1.expect(shared_1.extractFromUrl("pathname", "https://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("/asdf/qwer/last");
            chai_1.expect(shared_1.extractFromUrl(["path"], "https://www.example.com/asdf/qwer/last?key=val&b=2")).to.eql("/asdf/qwer/last");
            chai_1.expect(shared_1.extractFromUrl(["pathname"], "https://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("/asdf/qwer/last");
        });
        it("returns full host (e.g. www.example.ca:8080) if urlParts arg is 'host'", function () {
            chai_1.expect(shared_1.extractFromUrl("host", "https://www.example.com:8080/asdf/qwer/last?key=val&b=2")).to.eql("www.example.com:8080");
            chai_1.expect(shared_1.extractFromUrl(["host"], "http://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("www.example2.ca");
        });
        it("returns combinations of values in a conglomerate string", function () {
            chai_1.expect(shared_1.extractFromUrl(["protocol", "host"], "https://www.example.com:8080/asdf/qwer/last?key=val&b=2")).to.eql("https://www.example.com:8080");
            chai_1.expect(shared_1.extractFromUrl(["host", "pathname"], "http://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("www.example2.ca/asdf/qwer/last");
            chai_1.expect(shared_1.extractFromUrl(["pathname", "query"], "http://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("/asdf/qwer/last?key=val&b=2");
            chai_1.expect(shared_1.extractFromUrl(["host", "pathname", "query"], "http://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("www.example2.ca/asdf/qwer/last?key=val&b=2");
        });
        it("handles duplicate (overlapping) sections gracefully without duplications in returned string", function () {
            chai_1.expect(shared_1.extractFromUrl(["host", "hostname"], "http://www.example2.ca:8080/asdf/qwer/last?key=val&b=2")).to.eql("www.example2.ca:8080");
            chai_1.expect(shared_1.extractFromUrl(["host", "hostname", "pathname", "query"], "http://www.example2.ca/asdf/qwer/last?key=val&b=2")).to.eql("www.example2.ca/asdf/qwer/last?key=val&b=2");
        });
    });
    describe("isAbsoluteURL", function () {
        it("returns true for absolute HTTP URLs (http, https protocol) with domain", function () {
            chai_1.expect(shared_1.isAbsoluteURL("https://example.com")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("http://example.com/asdf")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("https://example.com/asdf/123")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("http://example.com/asdf/123?key=val")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("https://example.com/asdf/123#asdf")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("http://www.example.com/asdf/123#asdf")).to.be.true;
        });
        it("returns true for absolute non-http protocols (e.g. mailto, gopher) with domain", function () {
            chai_1.expect(shared_1.isAbsoluteURL("gopher://asdf.gopher")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("mailto://wat@example.com")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("ftp://gr.argh")).to.be.true;
        });
        it("returns false for relative URLs", function () {
            chai_1.expect(shared_1.isAbsoluteURL("example.com")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("example.com/asdf")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("example.com/asdf/123")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("example.com/asdf/123?key=val")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("example.com/asdf/123#asdf")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("www.example.com/asdf/123#asdf")).to.be.false;
        });
        it("returns false if only the protocol is present", function () {
            chai_1.expect(shared_1.isAbsoluteURL("http://")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("https://")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("gopher://")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("mailto://")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("ftp://")).to.be.false;
        });
        it("returns false if given null or undefined", function () {
            chai_1.expect(shared_1.isAbsoluteURL(null)).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL()).to.be.false;
        });
        it("returns false if // omitted for http, https protocol URLs", function () {
            chai_1.expect(shared_1.isAbsoluteURL("http:example.com")).to.be.false;
            chai_1.expect(shared_1.isAbsoluteURL("https:example.com/asdf/rer")).to.be.false;
        });
        it("returns true if // omitted from URLs using protocols besides http, https", function () {
            chai_1.expect(shared_1.isAbsoluteURL("mailto:asdf@example.com")).to.be.true;
            chai_1.expect(shared_1.isAbsoluteURL("gopher:1234asdf.example")).to.be.true;
        });
    });
});
