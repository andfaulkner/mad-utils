...
describe("validation sub-module", function () {
    ...
    describe("validateCanadaPostalCode", function () {
        ...
        it("Returns true given 3 character half-postal codes by default", function () {
            chai_1.expect(validation_1.validateCanadaPostalCode("K1H")).to.equal(true);
            chai_1.expect(validation_1.validateCanadaPostalCode("a7n")).to.equal(true);
        });
        it("Returns false given 3 character half-postal codes if false passed as 2nd param", function () {
            chai_1.expect(validation_1.validateCanadaPostalCode("K1H", false)).to.equal(false);
            chai_1.expect(validation_1.validateCanadaPostalCode("a7n", false)).to.equal(false);
        });
        ...
        it("Returns false given partial postal codes", function () {
            chai_1.expect(validation_1.validateCanadaPostalCode("V1")).to.equal(false);
            chai_1.expect(validation_1.validateCanadaPostalCode("V1", false)).to.equal(false);
            chai_1.expect(validation_1.validateCanadaPostalCode("K1H8E")).to.equal(false);
            chai_1.expect(validation_1.validateCanadaPostalCode("M7U A2")).to.equal(false);
        });
    });
});
