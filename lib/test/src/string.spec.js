...
describe("string sub-module", function () {
    ...
    describe('toSnakeCase', function () {
        ...
        it("Replaces apostrophes, quotes, ?, !, |, and , with '_'", function () {
            ...
            chai_1.expect(toSnakeCase('SomeTest"St`ring')).to.eql('some_test_st_ring');
            chai_1.expect(toSnakeCase('SomeTest`String')).to.eql('some_test_string');
            chai_1.expect(toSnakeCase('S"o\'me,Te|st|.Str!!,in?g!?!?,')).to.eql('s_o_me_te_st_str_in_g');
        });
        it("replaces ! \" # $ % & ' ( ) * + , - . / \u00A1 \u00A2 \u00A3 \u00A4 \u00A5 \u00A6 \u00A7 \u00A8 \u00A9 and \u00AA with '_'", function () {
            chai_1.expect(toSnakeCase("a!b\"c#d$e%f&g'h(i)j*k+l,m-n.o/p\u00A1q\u00A2r\u00A3s\u00A4t\u00A5u\u00A6v\u00A7w\u00A8x\u00A9y\u00AAz")).to.eql("a_b_c_d_e_f_g_h_i_j_k_l_m_n_o_p_q_r_s_t_u_v_w_x_y_z");
        });
        it("replaces \u00AB \u00AC \u00AE \u00AF \u00B0 \u00B1 \u00B2 \u00B3 \u00B4 \u00B6 \u00B7 \u00B8 \u00B9 \u00BA \u00BB \u00BC \u00BD \u00BE \u00BF with '_'", function () {
            chai_1.expect(toSnakeCase("a\u00ABb\u00ACc\u00AEd\u00AFe\u00B0f\u00B1g\u00B2h\u00B3i\u00B4j\u00B6k\u00B7l\u00B8m\u00B9n\u00BAo\u00BBp\u00BCq\u00BDr\u00BEs\u00BFt")).to.eql("a_b_c_d_e_f_g_h_i_j_k_l_m_n_o_p_q_r_s_t");
        });
        it("replaces : ; < = > ? @ [ ] \\ ^ _ ` { | } ~ \u00D7 \u00F7 with '_'", function () {
            chai_1.expect(toSnakeCase("a:b;c<d=e>f?g@h[i]j\\k^l_m`n{o|p}q~r\u00D7s\u00F7t")).to.eql("a_b_c_d_e_f_g_h_i_j_k_l_m_n_o_p_q_r_s_t");
        });
        it("surrounds \u00DE \u00FE \u00F8 \u00B5 \u00DF with '-', converting \u00DE to \u00FE in the process", function () {
            chai_1.expect(toSnakeCase("a\u00DEb\u00FEc\u00F8d\u00B5e\u00DFf")).to.eql("a_\u00FE_b_\u00FE_c_\u00F8_d_\u00B5_e_\u00DF_f");
            chai_1.expect(toSnakeCase(" a\u00DE b\u00FEc \u00F8d\u00B5  e\u00DFf ")).to.eql("a_\u00FE_b_\u00FE_c_\u00F8_d_\u00B5_e_\u00DF_f");
            chai_1.expect(toSnakeCase("some\u00B5Of")).to.eql("some_\u00B5_of");
        });
        ...
        it("Handles foreign characters", function () {
            chai_1.expect(toSnakeCase('SomeTestȘtring')).to.eql('some_test_ștring');
            chai_1.expect(toSnakeCase('ȘōmëŤèsťȘtrîn̈g')).to.eql('șōmë_ťèsť_ștrîn̈g');
            chai_1.expect(toSnakeCase('Șōmë tèsť ștrîn̈ g')).to.eql('șōmë_tèsť_ștrîn̈_g');
        });
    });
    describe('toDashCase', function () {
        node_1.expectFunctionExists(matchesIgnoreCase);
        node_1.expectFunctionExists(toDashCase);
        it("returns empty string given null", function () {
            chai_1.expect(toDashCase(null)).to.eql('');
        });
        it("returns empty string given empty string or null", function () {
            chai_1.expect(toDashCase('')).to.eql('');
        });
        it("returns empty string given just -, _, or space", function () {
            chai_1.expect(toDashCase(' ')).to.eql('');
            chai_1.expect(toDashCase('_')).to.eql('');
            chai_1.expect(toDashCase('-')).to.eql('');
        });
        it("replaces spaces with -", function () {
            chai_1.expect(toDashCase('some test string')).to.eql('some-test-string');
            chai_1.expect(toDashCase('  some test string  ')).to.eql('some-test-string');
            chai_1.expect(toDashCase('  so  me test stri    ng  ')).to.eql('so-me-test-stri-ng');
        });
        it("returns dash-case form of camelCase string", function () {
            chai_1.expect(toDashCase('someTestString')).to.eql('some-test-string');
        });
        ...
        it("returns dash-case form of string preceded by underscores", function () {
            chai_1.expect(toDashCase('_____SomeTestString')).to.eql('some-test-string');
        });
        ...
        it("returns dash-case form of string preceded by dashes & underscores", function () {
            chai_1.expect(toDashCase('-___-SomeTestString')).to.eql('some-test-string');
        });
        it("Replaces apostrophes, quotes, ?, !, |, and , with '-'", function () {
            chai_1.expect(toDashCase('Some,TestString?!?')).to.eql('some-test-string');
            chai_1.expect(toDashCase("SomeTest'St`ring")).to.eql('some-test-st-ring');
            chai_1.expect(toDashCase('SomeTest"String')).to.eql('some-test-string');
            chai_1.expect(toDashCase('SomeTest`String')).to.eql('some-test-string');
            chai_1.expect(toDashCase('S"o\'me,Te|st|.Str!!,in?g!?!?,')).to.eql('s-o-me-te-st-str-in-g');
        });
        it("replaces ! \" # $ % & ' ( ) * + , - . / \u00A1 \u00A2 \u00A3 \u00A4 \u00A5 \u00A6 \u00A7 \u00A8 \u00A9 and \u00AA with '-'", function () {
            chai_1.expect(toDashCase("a!b\"c#d$e%f&g'h(i)j*k+l,m-n.o/p\u00A1q\u00A2r\u00A3s\u00A4t\u00A5u\u00A6v\u00A7w\u00A8x\u00A9y\u00AAz")).to.eql("a-b-c-d-e-f-g-h-i-j-k-l-m-n-o-p-q-r-s-t-u-v-w-x-y-z");
        });
        it("replaces \u00AB \u00AC \u00AE \u00AF \u00B0 \u00B1 \u00B2 \u00B3 \u00B4 \u00B6 \u00B7 \u00B8 \u00B9 \u00BA \u00BB \u00BC \u00BD \u00BE \u00BF with '-'", function () {
            chai_1.expect(toDashCase("a\u00ABb\u00ACc\u00AEd\u00AFe\u00B0f\u00B1g\u00B2h\u00B3i\u00B4j\u00B6k\u00B7l\u00B8m\u00B9n\u00BAo\u00BBp\u00BCq\u00BDr\u00BEs\u00BFt")).to.eql("a-b-c-d-e-f-g-h-i-j-k-l-m-n-o-p-q-r-s-t");
        });
        it("replaces : ; < = > ? @ [ ] \\ ^ _ ` { | } ~ \u00D7 \u00F7 with '-'", function () {
            chai_1.expect(toDashCase("a:b;c<d=e>f?g@h[i]j\\k^l_m`n{o|p}q~r\u00D7s\u00F7t")).to.eql("a-b-c-d-e-f-g-h-i-j-k-l-m-n-o-p-q-r-s-t");
        });
        it("surrounds \u00DE \u00FE \u00F8 \u00B5 \u00DF with '-', converting \u00DE to \u00FE in the process", function () {
            chai_1.expect(toDashCase("a\u00DEb\u00FEc\u00F8d\u00B5e\u00DFf")).to.eql("a-\u00FE-b-\u00FE-c-\u00F8-d-\u00B5-e-\u00DF-f");
            chai_1.expect(toDashCase(" a\u00DE b\u00FEc \u00F8d\u00B5  e\u00DFf ")).to.eql("a-\u00FE-b-\u00FE-c-\u00F8-d-\u00B5-e-\u00DF-f");
            chai_1.expect(toDashCase("some\u00B5Of")).to.eql("some-\u00B5-of");
        });
        ...
        it("Handles foreign characters", function () {
            chai_1.expect(toDashCase('SomeTestȘtring')).to.eql('some-test-ștring');
            chai_1.expect(toDashCase('ȘōmëŤèsťȘtrîn̈g')).to.eql('șōmë-ťèsť-ștrîn̈g');
            chai_1.expect(toDashCase('Șōmë tèsť ștrîn̈ g')).to.eql('șōmë-tèsť-ștrîn̈-g');
        });
    });
