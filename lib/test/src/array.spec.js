...
describe("array sub-module", function () {
    ...
    test_1.expectNonEmptyObjectExists(browser_2.m_.array, 'array (from m_ top-level namespace)');
    ...
    describe("-- #append", function () {
        it("-- exists : #append", function () {
            chai_1.expect(browser_2.m_.array.append).to.exist;
    ...
    describe("-- #first", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.first).to.exist;
            chai_1.expect(shared_1.first).to.exist;
        });
        it("returns first item in an array", function () {
            chai_1.expect(browser_2.m_.array.first(['one'])).to.eql('one');
            chai_1.expect(browser_2.m_.array.first(['one', 'two'])).to.eql('one');
        });
        it("returns undefined if array is empty", function () {
            chai_1.expect(browser_2.m_.array.first([])).to.be.undefined;
        });
    });
    describe("-- #second", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.second).to.exist;
            chai_1.expect(shared_1.second).to.exist;
        });
        it("returns second item in an array", function () {
            chai_1.expect(browser_2.m_.array.second(['one', 'two'])).to.eql('two');
            chai_1.expect(browser_2.m_.array.second(['one', 'two', 'three'])).to.eql('two');
        });
        it("returns undefined if array is empty or only has one item", function () {
            chai_1.expect(browser_2.m_.array.second([])).to.be.undefined;
            chai_1.expect(browser_2.m_.array.second(['one'])).to.be.undefined;
        });
    });
    describe("-- #third", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.third).to.exist;
            chai_1.expect(shared_1.third).to.exist;
        });
        it("returns third item in an array", function () {
            chai_1.expect(browser_2.m_.array.third(['one', 'two', 'three'])).to.eql('three');
            chai_1.expect(browser_2.m_.array.third(['one', 'two', 'three', 'four'])).to.eql('three');
        });
        it("returns undefined if array is empty or only has 1 or 2 items", function () {
            chai_1.expect(browser_2.m_.array.third([])).to.be.undefined;
            chai_1.expect(browser_2.m_.array.third(['one'])).to.be.undefined;
            chai_1.expect(browser_2.m_.array.third(['one', 'two'])).to.be.undefined;
        });
    });
    describe("-- #last", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.last).to.exist;
            chai_1.expect(shared_1.last).to.exist;
        });
        it("returns last item in an array", function () {
            chai_1.expect(browser_2.m_.array.last(['one'])).to.eql('one');
            chai_1.expect(browser_2.m_.array.last(['one', 'two', 'three'])).to.eql('three');
            chai_1.expect(browser_2.m_.array.last(['one', 'two', 'three', 'four'])).to.eql('four');
        });
        it("returns undefined if array is empty", function () {
            chai_1.expect(browser_2.m_.array.last([])).to.be.undefined;
        });
    });
    describe("-- #secondLast", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.secondLast).to.exist;
            chai_1.expect(shared_1.secondLast).to.exist;
        });
        it("returns second last item in an array", function () {
            chai_1.expect(browser_2.m_.array.secondLast(['one', 'two'])).to.eql('one');
            chai_1.expect(browser_2.m_.array.secondLast(['one', 'two', 'three'])).to.eql('two');
            chai_1.expect(browser_2.m_.array.secondLast(['one', 'two', 'three', 'four'])).to.eql('three');
        });
        it("returns undefined if array is empty", function () {
            chai_1.expect(browser_2.m_.array.secondLast([])).to.be.undefined;
        });
    });
    describe("-- #thirdLast", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.thirdLast).to.exist;
            chai_1.expect(shared_1.thirdLast).to.exist;
        });
        it("returns third last item in an array", function () {
            chai_1.expect(browser_2.m_.array.thirdLast(['one', 'two', 'three'])).to.eql('one');
            chai_1.expect(browser_2.m_.array.thirdLast(['one', 'two', 'three', 'four'])).to.eql('two');
            chai_1.expect(browser_2.m_.array.thirdLast(['one', 'two', 'three', 'four', 'five'])).to.eql('three');
        });
        it("returns undefined if array is empty", function () {
            chai_1.expect(browser_2.m_.array.thirdLast([])).to.be.undefined;
        });
    });
    describe("-- #last2", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.last2).to.exist;
            chai_1.expect(shared_1.last2).to.exist;
        });
        it("returns last 2 items in an array (in array form)", function () {
            chai_1.expect(browser_2.m_.array.last2(['one', 'two'])).to.eql(['one', 'two']);
            chai_1.expect(browser_2.m_.array.last2(['one', 'two', 'three'])).to.eql(['two', 'three']);
        });
        it("returns empty array if source array is empty", function () {
            chai_1.expect(browser_2.m_.array.last2([])).to.eql([]);
        });
        it("If source array has 1 item, return new array w/ 1 source array item", function () {
            chai_1.expect(browser_2.m_.array.last2(['one'])).to.eql(['one']);
        });
    });
    describe("-- #last3", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.last3).to.exist;
            chai_1.expect(shared_1.last3).to.exist;
        });
        it("returns last 3 items in an array (in array form)", function () {
            chai_1.expect(browser_2.m_.array.last3(['one', 'two', 'three'])).to.eql(['one', 'two', 'three']);
            chai_1.expect(browser_2.m_.array.last3(['one', 'two', '3', 'four'])).to.eql(['two', '3', 'four']);
        });
        it("returns empty array if source array is empty", function () {
            chai_1.expect(browser_2.m_.array.last3([])).to.eql([]);
        });
        it("If source array has less than 3 items, return clone of original array", function () {
            chai_1.expect(browser_2.m_.array.last3(['one'])).to.eql(['one']);
            chai_1.expect(browser_2.m_.array.last3(['one', 'two'])).to.eql(['one', 'two']);
        });
    });
    describe("-- #lastN", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.lastN).to.exist;
            chai_1.expect(shared_1.lastN).to.exist;
        });
        it("returns requested # of items from end of source array (in array form)", function () {
            chai_1.expect(browser_2.m_.array.lastN(['one', 'two', 'three'], 1)).to.eql(['three']);
            chai_1.expect(browser_2.m_.array.lastN(['one', 'two', 'three'], 2)).to.eql(['two', 'three']);
        });
        it("returns empty array if source array is empty", function () {
            chai_1.expect(browser_2.m_.array.lastN([], 1)).to.eql([]);
            chai_1.expect(browser_2.m_.array.lastN([], 5)).to.eql([]);
        });
        it("If src array has less than requested # of items, return src array clone", function () {
            chai_1.expect(browser_2.m_.array.lastN(['one'], 2)).to.eql(['one']);
            chai_1.expect(browser_2.m_.array.lastN(['one', 'two'], 12)).to.eql(['one', 'two']);
        });
    });
    describe("-- #first2", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.first2).to.exist;
            chai_1.expect(shared_1.first2).to.exist;
        });
        it("returns first 2 items in an array (in array form)", function () {
            chai_1.expect(browser_2.m_.array.first2(['one', 'two'])).to.eql(['one', 'two']);
            chai_1.expect(browser_2.m_.array.first2(['one', 'two', 'three'])).to.eql(['one', 'two']);
        });
        it("returns empty array if source array is empty", function () {
            chai_1.expect(browser_2.m_.array.first2([])).to.eql([]);
        });
        it("If source array has 1 item, return new array w/ 1 source array item", function () {
            chai_1.expect(browser_2.m_.array.first2(['one'])).to.eql(['one']);
        });
    });
    describe("-- #first3", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.first3).to.exist;
            chai_1.expect(shared_1.first3).to.exist;
        });
        it("returns first 3 items in an array (in array form)", function () {
            chai_1.expect(browser_2.m_.array.first3(['one', 'two', 'three'])).to.eql(['one', 'two', 'three']);
            chai_1.expect(browser_2.m_.array.first3(['one', 'two', '3', 'four'])).to.eql(['one', 'two', '3']);
        });
        it("returns empty array if source array is empty", function () {
            chai_1.expect(browser_2.m_.array.first3([])).to.eql([]);
        });
        it("If source array has less than 3 items, return clone of original array", function () {
            chai_1.expect(browser_2.m_.array.first3(['one'])).to.eql(['one']);
            chai_1.expect(browser_2.m_.array.first3(['one', 'two'])).to.eql(['one', 'two']);
        });
    });
    describe("-- #firstN", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.firstN).to.exist;
            chai_1.expect(shared_1.firstN).to.exist;
        });
        it("returns requested # of items from start of source array (in array form)", function () {
            chai_1.expect(browser_2.m_.array.firstN(['one', 'two', 'three'], 1)).to.eql(['one']);
            chai_1.expect(browser_2.m_.array.firstN(['one', 'two', 'three'], 2)).to.eql(['one', 'two']);
        });
        it("returns empty array if source array is empty", function () {
            chai_1.expect(browser_2.m_.array.firstN([], 1)).to.eql([]);
            chai_1.expect(browser_2.m_.array.firstN([], 5)).to.eql([]);
        });
        it("If src array has less than requested # of items, return src array clone", function () {
            chai_1.expect(browser_2.m_.array.firstN(['one'], 2)).to.eql(['one']);
            chai_1.expect(browser_2.m_.array.firstN(['one', 'two'], 12)).to.eql(['one', 'two']);
        });
    });
    describe("-- #arrayN", function () {
        it("exists", function () {
            chai_1.expect(browser_2.m_.array.arrayN).to.exist;
            chai_1.expect(shared_1.arrayN).to.exist;
        });
        it("returns given length array of empty items if run w 1 integer arg", function () {
            chai_1.expect(browser_2.m_.array.arrayN(1)).to.be.length(1);
            chai_1.expect(browser_2.m_.array.arrayN(3)).to.be.length(3);
            chai_1.expect(browser_2.m_.array.arrayN(3)).to.eql([undefined, undefined, undefined]);
            chai_1.expect(browser_2.m_.array.arrayN(1)).to.eql([undefined]);
        });
        it("returns array containing given number of copies of provided item if run w 2 args: 1 " +
            ...
            chai_1.expect(browser_2.m_.array.arrayN(1, 'asdf')).to.be.length(1);
            chai_1.expect(browser_2.m_.array.arrayN(1, 'asdf')).to.eql(['asdf']);
            chai_1.expect(browser_2.m_.array.arrayN(2, {})).to.be.length(2);
            chai_1.expect(browser_2.m_.array.arrayN(2, { ok: 'yep' })).to.eql([{ ok: 'yep' }, { ok: 'yep' }]);
        ...
        it("fills with null if value given is null, & undefined if val is undefined", function () {
            chai_1.expect(shared_1.arrayN(2, null)).to.be.length(2);
            chai_1.expect(shared_1.arrayN(2, null)).to.eql([null, null]);
            chai_1.expect(shared_1.arrayN(2, undefined)).to.be.length(2);
            chai_1.expect(shared_1.arrayN(2, undefined)).to.eql([undefined, undefined]);
        });
    });
    describe("-- #rmAllFalsy", function () {
        ...
        it("given arr w/ multiple falsy vals, returns new array w/ the falsy vals rm", function () {
            ...
            chai_1.expect(shared_1.rmAllFalsy([false, false, 'one', false, 'three', false])).to.eql([
                'one',
                'three',
            ]);
            chai_1.expect(shared_1.rmAllFalsy(['one', null, 'three', null, 'five', undefined])).to.eql([
                'one',
                'three',
                'five',
            ]);
            chai_1.expect(shared_1.rmAllFalsy(['', '', 'one', 0, 'three', null, 'five', undefined, false])).to.eql([
                'one',
                'three',
                'five',
            ]);
        ...
        it("leaves 0 in place if 'keep0' given as 2nd prop", function () {
            chai_1.expect(shared_1.rmAllFalsy([undefined, 'a', 0, null], 'keep0')).to.eql(['a', 0]);
            chai_1.expect(shared_1.rmAllFalsy([0], 'keep0')).to.eql([0]);
        });
        it("leaves '' in place if 'keepStr' given as 2nd prop", function () {
            chai_1.expect(shared_1.rmAllFalsy([undefined, 'a', '', null], 'keepStr')).to.eql(['a', '']);
            chai_1.expect(shared_1.rmAllFalsy([''], 'keepStr')).to.eql(['']);
        });
        it("only removes undefined & null if 'nullUndef' given as 2nd prop", function () {
            var outArr = ['a', '', NaN, 0, 'b'];
            chai_1.expect(shared_1.rmAllFalsy([undefined, 'a', '', NaN, 0, 'b', null], 'nullUndef')).to.eql(outArr);
            chai_1.expect(shared_1.rmAllFalsy([undefined, '', null, NaN], 'nullUndef')).to.eql(['', NaN]);
        });
    });
    ...
    describe("-- #splitLines", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.splitLines);
    ...
    describe("-- without.last", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutLast);
    ...
    describe("-- without.last2", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutLast2);
    ...
    describe("-- without.last3", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutLast3);
    ...
    describe("-- without.first", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutFirst);
    ...
    describe("-- without.first2", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutFirst2);
    ...
    describe("-- without.first3", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutFirst3);
    ...
    describe("-- without.firstN", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutFirstN);
    ...
    describe("-- without.lastN", function () {
        ...
        test_1.expectFunctionExists(browser_2.m_.array.withoutLastN);
    ...
    describe("-- removeMatches", function () {
        ...
        test_1.expectFunctionExists(shared_1.removeMatches);
        test_1.expectFunctionExists(browser_2.m_.array.removeMatches);
        ...
        it("Excludes items in 2nd arg from 1st arg even if duplicates found in 2nd arg", function () {
            chai_1.expect(shared_1.removeMatches([null, 'two', 'three', 4], [null, 'two', 'two', 4])).to.eql([
                'three',
            ]);
            chai_1.expect(shared_1.removeMatches([null, 'two', 'three', 4], [null, 4, 'two', 'three', 4])).to.eql([]);
            ...
            chai_1.expect(shared_1.removeMatches([1, 2, 3, 4], [1, 2, 4, 4, 4, 4, 4, 4, 4, 2, 2, 2, 2])).to.eql([
                3,
            ]);
        });
    });
    describe("-- arrayRemove", function () {
        ...
        var testArr2 = ['a', 'b', 'c', 'd', 'b', 'e', 'b', 'b'];
        var testArr1Return;
        var testArr2Return;
        before(function () {
            testArr1Return = shared_1.arrayRemove(testArr1, 'c');
            testArr2Return = shared_1.arrayRemove(testArr2, 'b');
        });
        it("Removes all matches of given item (needle) from array (haystack) mutatively", function () {
            chai_1.expect(testArr1).to.eql(['a', 'b', 'd', 'e']);
            chai_1.expect(testArr2).to.eql(['a', 'c', 'd', 'e']);
        });
        it("Returns new array - with all items removed", function () {
            chai_1.expect(testArr1Return).to.eql(['a', 'b', 'd', 'e']);
            chai_1.expect(testArr2Return).to.eql(['a', 'c', 'd', 'e']);
            chai_1.expect(testArr1Return).to.eql(testArr1);
            chai_1.expect(testArr2Return).to.eql(testArr2);
        });
        it("Returns original array and leaves it untouched if no match found", function () {
            var testArr3 = [1, 2, 3, 4];
            var testArr3Ret = shared_1.arrayRemove(testArr3, 6);
            chai_1.expect(testArr3).to.eql([1, 2, 3, 4]);
            chai_1.expect(testArr3Ret).to.eql([1, 2, 3, 4]);
        });
        it("Handles empty arrays", function () {
            var testArr4 = [];
            var testArr4Ret = shared_1.arrayRemove([], 'asdf');
            chai_1.expect(testArr4).to.eql([]);
            chai_1.expect(testArr4Ret).to.eql([]);
        });
    });
    ...
    describe("countOccurrences", function () {
        it("returns number of occurrences of given value in array", function () {
            chai_1.expect(shared_1.countOccurrences(7, [7, 9, 7, 'asdf', 7, 1, 7, null, 7])).to.eql(5);
        });
        it("returns number of occurrences of given character in string", function () {
            chai_1.expect(shared_1.countOccurrences("a", "asdf asdf")).to.eql(2);
        });
        it("returns number of null values in array of null values", function () {
            chai_1.expect(shared_1.countOccurrences(null, [null, null, null])).to.eql(3);
        });
        it("returns 0 given an empty array", function () {
            chai_1.expect(shared_1.countOccurrences(7, [])).to.eql(0);
            chai_1.expect(shared_1.countOccurrences(undefined, [])).to.eql(0);
            chai_1.expect(shared_1.countOccurrences('asdf', [])).to.eql(0);
        });
        it("returns 0 given an empty string", function () {
            chai_1.expect(shared_1.countOccurrences('', '')).to.eql(0);
            chai_1.expect(shared_1.countOccurrences('a', '')).to.eql(0);
        });
    });
    describe('removeDuplicates', function () {
        it("Returns a new version of an array with all duplicates removed", function () {
            var testOutArr = shared_1.removeDuplicates([1, 1, 2, 3, 1, 1, 2, 1, 3]);
            chai_1.expect(testOutArr).to.have.members([1, 2, 3]);
            chai_1.expect(testOutArr)
                .to.be.an('array')
                .with.lengthOf(3);
        });
        it("Returns a new version of a string with all duplicate chars removed", function () {
            var testOutStr = shared_1.removeDuplicates('aaaasssssddddfffff');
            chai_1.expect(testOutStr).to.eql('asdf');
            chai_1.expect(testOutStr)
                .to.be.a('string')
                .with.lengthOf(4);
        });
    });
    describe("sample", function () {
        it("given a string, gets a random character", function () {
            chai_1.expect(shared_1.sample('qwerty')).to.match(/^[qwerty]$/);
        });
        it("given an empty string, returns undefined", function () {
            chai_1.expect(shared_1.sample('')).to.be.undefined;
        });
        it("given an array, gets a random value", function () {
            chai_1.expect(shared_1.sample([1, 2, 3, 4, 5])).to.be.oneOf([1, 2, 3, 4, 5]);
        });
        it("given an empty array, returns undefined", function () {
            chai_1.expect(shared_1.sample([])).to.be.undefined;
        });
        it("given an object, gets a random key-value pair (as [key, value])", function () {
            chai_1.expect([['a', 1], ['b', 2], ['c', 3]]).to.deep.include(shared_1.sample({ a: 1, b: 2, c: 3 }));
        });
        it("given an empty object (with no keys), returns undefined", function () {
            chai_1.expect(shared_1.sample({})).to.be.undefined;
        });
        it("given a Map, gets a random array pair from it", function () {
            var myMap = new Map([['a', 1], ['b', 2], ['c', 3], ['d', 4], ['e', 5]]);
            var mySample = shared_1.sample(myMap);
            chai_1.expect(mySample).to.have.length(2);
            chai_1.expect(mySample[0]).to.match(/^[abcde]$/);
            chai_1.expect(mySample[1].toString()).to.match(/^[12345]$/);
        });
        it("given an empty Map, returns an empty array", function () {
            var myMap2 = new Map();
            console.log('myMap');
            console.log(myMap2);
            var mySample2 = shared_1.sample(myMap2);
            console.log('mySample');
            console.log(mySample2);
            chai_1.expect(mySample2).to.have.length(0);
            chai_1.expect(mySample2).to.eql([]);
        });
        it("given a Set, gets a random item from it", function () {
            var mySet = new Set(['a', 'b', 'c', 'd']);
            chai_1.expect(shared_1.sample(mySet)).to.match(/^[abcd]$/);
        });
        it("given an empty Set, returns []", function () {
            var mySet = new Set();
            chai_1.expect(shared_1.sample(mySet)).to.eql([]);
        });
    });
    describe("flatten", function () {
        it("given empty array, returns empty array", function () {
            chai_1.expect(shared_1.flatten([])).to.eql([]);
        });
        it("given a single-level array, returns input as-is", function () {
            chai_1.expect(shared_1.flatten([1])).to.eql([1]);
            chai_1.expect(shared_1.flatten([1, 2])).to.eql([1, 2]);
            chai_1.expect(shared_1.flatten([1, 2, 3, 4, 5, 6, 7, 8, 9])).to.eql([1, 2, 3, 4, 5, 6, 7, 8, 9]);
        });
        it("given an array of 1-deep nested arrays, returns a single-level array", function () {
            chai_1.expect(shared_1.flatten([1, 2, [3, 4]])).to.eql([1, 2, 3, 4]);
        });
        it("given an array of 2-deep nested arrays, returns a single-level array", function () {
            chai_1.expect(shared_1.flatten([1, 2, [3, 4, [5, 6]]])).to.eql([1, 2, 3, 4, 5, 6]);
            chai_1.expect(shared_1.flatten([1, [[2, 3], 4, [5, 6], [7]], [[8]]])).to.eql([1, 2, 3, 4, 5, 6, 7, 8]);
        });
        it("given an array of 3-deep nested arrays, returns a single-level array", function () {
            chai_1.expect(shared_1.flatten([[[[1, 2], 3], [4], 5], [6, [[7]]], 8])).to.eql([
                1,
                2,
                3,
                4,
