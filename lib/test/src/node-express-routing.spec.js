...
describe("error sub-module", function () {
    ...
    describe("isSupportedLang", function () {
        it("returns true if given 'en' or 'fr' (with default supportedLangs)", function () {
            chai_1.expect(node_1.isSupportedLang('en')).to.be.true;
            chai_1.expect(node_1.isSupportedLang('fr')).to.be.true;
        });
        ...
        it("Allows a custom language set to be set, returns true if given val from custom set)", function () {
            chai_1.expect(node_1.isSupportedLang('swahili', ['swahili', 'zulu', 'en'])).to.be.true;
            chai_1.expect(node_1.isSupportedLang('zulu/', ['swahili', 'zulu', 'en'])).to.be.true;
        });
        it("Returns false if given value other than 'en' or 'fr' (default supportedLangs)", function () {
            chai_1.expect(node_1.isSupportedLang('swahili')).to.be.false;
            chai_1.expect(node_1.isSupportedLang('zulu/')).to.be.false;
        });
        it("Returns false if given a value not in the given supportedLangs list", function () {
            chai_1.expect(node_1.isSupportedLang('en', ['swahili', 'zulu'])).to.be.false;
            chai_1.expect(node_1.isSupportedLang('swahili/', ['en', 'zulu'])).to.be.false;
        });
    });
});
