...
exports.splitLines = function (str, opts) {
    ...
    return opts.preserveEmptyLines
        ? str.toString().split('\n')
...
exports.matchAny = function (matchVals) { return function (valToFind) {
    ...
    var comparisonVal = isValToFindObj ? JSON.stringify(valToFind) : valToFind;
    var valToFileIsNaN = typeof valToFind === 'number' && isNaN(valToFind);
    var didMatch = matchVals.some(function (val) {
        ...
        var curMatch = isValToFindObj ? comparisonVal === (val && JSON.stringify(val))
            : comparisonVal === val;
