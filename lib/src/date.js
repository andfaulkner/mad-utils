...
function isLeapYear(year) {
    var yearClean = typeof  === 'string' ? parseInt(year) : year;
    if (yearClean instanceof Error || !types_iso_1.isInt(yearClean) || isNaN(yearClean)) {
 ...
 */
var _invalidDateStringHandler = function (date) {
...
var _dateStringWithMonthTextToMomentFallback = function (date, dateParts, locale, fallbackFormat) {
    ...
    if (dateParts.length !== 3) {
        if (typeof fallbackFormat === 'string') {
            ...
            if (retVal.isValid && retVal.isValid())
                return retVal;
        }
    ...
    var dateOfMonth = (dateParts[2].length === 1) ? "0" + dateParts[2] : dateParts[2];
    ...
    if (fallbackFormat === true &&
        ...
        (dateOfMonth.length === 2 && !isNaN(dateOfMonth))) {
...
exports.dateStringWithMonthTextToMoment = function (date, opts) {
    ...
    var monthDatePtsIdx = dateParts.findIndex(function (pt) { return monthStrs.some(function (moStr) { return pt.includes(moStr); }); });
    ...
    var retVal = moment({ date: dateOfMonth, month: month - 1, year: year }, undefined, lc);
