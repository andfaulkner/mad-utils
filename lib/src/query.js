 ...
 */
var queryParamsDef = window.location.search || '';
/****************************************** QUERY PARAMS ******************************************/
/**
...
function getLangFromUrlPathname(urlPath, supportedLangs, defaultLang) {
    ...
    var cleanUrlPath = typeof urlPath === 'string' ? urlPath : window.location.pathName;
...
function urlMinusQueryParams(url) {
    var cleanUrl = typeof url === 'string' ? url : window.location.href;
...
function urlProtocolString(url) {
    var cleanUrl = typeof url === 'string' ? url : window.location.href;
    return string_1.matchFirst(cleanUrl, /^https?:\/\//g);
}
...
function swapLastURLPath(newPathVal, url) {
    var cleanUrl = typeof url === 'string' ? url : window.location.href;
    var queryStr = urlGetQuery(cleanUrl);
    var urlSansLastPath = urlMinusLastPath(cleanUrl);
 ...
 */
function swapMatchingURLPath(newPathVal, pathToMatch, url) {
    var cleanUrl = typeof url === 'string' ? url : window.location.href;
    var httpStr = urlProtocolString(cleanUrl);
    var queryStr = urlGetQuery(cleanUrl);
    var urlParts = urlWithoutProtocol(urlMinusQueryParams(cleanUrl)).split('/');
    var urlPartsSwapped = urlParts.map(function (urlPt, idx) { return ((urlPt.match(pathToMatch)) && idx !== 0) ? newPathVal : urlPt; });
    return "" + httpStr + urlPartsSwapped.join('/') + "?" + queryStr;
}
exports.swapMatchingURLPath = swapMatchingURLPath;
