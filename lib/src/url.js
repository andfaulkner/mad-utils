...
exports.urlGetQuery = exports.urlQuery;
exports.getQueryString = exports.urlQuery;
exports.getQueryParamString = exports.urlQuery;
exports.urlQueryString = exports.urlQuery;
exports.urlGetQueryString = exports.urlQuery;
exports.urlGetQueryParamString = exports.urlQuery;
/**
 ...
 */
exports.urlPathnameWithQuery = function (url) {
    var cleanUrl = url || global.location.href;
    if (!url)
        return "";
    return exports.normalizeURLPathname(exports.urlWithoutProtocol(cleanUrl).replace(/^[^\/]+(?=\/)/, ""));
};
/**
 ...
 * @return {string} Pathname and query from given URL (or current URL)
 */
exports.urlPathname = function (url) {
    var cleanUrl = url || global.location.href;
    if (!url)
        return "";
    return exports.normalizeURLPathname(exports.urlWithoutProtocol(cleanUrl)
        .replace(/^[^\/]+(?=\/)/, "")
        .replace(/\?.+$/, ""));
};
/**
...
exports.urlMinusLastPath = function (url, excludeQuery) {
    ...
    var queryStr = exports.urlQuery(cleanUrl);
 ...
 *          // => `?key=val&b=2`
 */
exports.extractFromUrl = function (urlParts, url) {
    if (url === void 0) { url = global.location.href; }
    var outUrl = "";
    var noProtocolUrl = exports.urlWithoutProtocol(url);
    var host = noProtocolUrl.split("/")[0];
    var port = (host.match(/:[0-9]+/g) || [""])[0];
    var cleanParts = typeof urlParts === "string" ? [urlParts] : urlParts;
    if (cleanParts.some(function (part) { return part === "protocol"; })) {
        outUrl += exports.urlProtocolString(url);
    }
    if (cleanParts.some(function (part) { return part === "host"; })) {
        outUrl += host;
    }
    else {
        if (cleanParts.some(function (part) { return part === "hostname"; })) {
            var hostname = host.replace(port, "");
            outUrl += hostname;
        }
        if (cleanParts.some(function (part) { return part === "port"; })) {
            outUrl += port;
        }
    }
    if (cleanParts.some(function (part) { return part === "pathname" || part === "path"; })) {
        outUrl += exports.urlPathname(noProtocolUrl);
    }
    if (cleanParts.some(function (part) { return part === "query"; })) {
        var query = exports.urlQuery(url);
        outUrl += query ? "?" + query : "";
    }
    return outUrl;
};
...
exports.isAbsoluteURL = function (url) {
    return !!url && !!url.match(/^[a-z][a-z0-9+.-]*:\/\/[a-zA-Z0-9_]+/i);
};
