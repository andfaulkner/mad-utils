...
describe(`stream sub-module`, function() {
    ...
    expectFunctionExists(
        CharInputStream_browser,
        'CharInputStream exported from browser submodule'
    );
    ...
    describe('CharInputStream', function() {
        ...
        it(`peekMulti(4) peeks ahead 4`, function() {
            const localInput = new CharInputStream('Hello new values!');
            expect(localInput.peekMulti(4)).to.eql('Hell');
        });
    });
});
